...
kB = 1.381 * 10**(-23) #J/K
t_0 = 100 # K
R = 8.20573 #cubic-meter.atm/mol/K
g = 9.81 #m/s^2
...
co2_molecular_weight = [44,0,0,0,0,0]
...
ch4_beta_0s = [0.8698,0.06973,0.35429,0.68598]
ch4_omega_0s = [21.0,56.0,60.0,45.0]
ch4_molecular_weight = [16,0,0,0]
...
def psi_fun(temperature,matrix): # temperature, degrees of freedom, matrix
    ...
    for p in range(0,np.size(matrix,0)):
        ...
        psi_num_prev = 0 #previous numerator of equation for psi
        psi_den_prev = 0 #previous denominator of equation for psi
...
def phi_fun(temperature,matrix): # temperature, degrees of freedom, matrix
    ...
    for p in range(0,np.size(matrix,0)):
        ...
        phi_num_prev = 0 #previous numerator of equation for phi
        phi_den_prev = 0 #previous denominator of equation for phi
...
def sD(temperature,matrix,wavenumber):
    global alpha
    global omega
    sDs = np.zeros((np.size(matrix,0),1)) # array initialization
    alphas = alpha(temperature,matrix)
    omegas = omega(temperature,matrix)
    for p in range(0,np.size(matrix,0)):

        band_center = matrix[p,2]
        sDs[p] = alphas[p]/omegas[p]*\
                np.exp(-2*abs(wavenumber-band_center)/omegas[p])
##        if abs(wavenumber-band_center)/omegas[p]>7:
##            sDs[p]=0
##        else:
##            sDs[p] = alphas[p]/omegas[p]*\
##                     np.exp(-2*abs(wavenumber-band_center)/omegas[p])
...
def P_e(pressure,partial_pressure,matrix):
    P_es = np.zeros((np.size(matrix,0),1)) # array initialization
    dof = int(matrix[0,0])
    for p in range(0,np.size(matrix,0)):
        P_es[p]=(pressure+partial_pressure*(matrix[p,3+dof]-1))**matrix[p,4+dof]

    return P_es
    
dvs_co2 = np.zeros((50,1))
for i in range(0,50):
    dvs_co2[i]=500+100*i

dvs_ch4 = np.zeros((60,1))
for i in range(0,60):
    dvs_ch4[i]=1000+100*i


def alpha_vs(matrix,dvs,mole_fraction,height):
    alpha_vs_p = np.zeros((np.size(matrix,0),1)) # array initialization
    alpha_vs = np.zeros((np.size(dvs,0),1))
    global sD
    global beta
    global P_e

    temperature = 280-height*0.005636
    pressure = np.exp(-g*0.0289644*height/(8.314*temperature))
    partial_pressure = pressure*mole_fraction
    P_es = P_e(pressure,partial_pressure,matrix)
    betas = beta(temperature, matrix)
    dof = int(matrix[0,0])
    Mw = matrix[0,8+3*dof]
    density = Mw*pressure/R/temperature

    for i in range(0,np.size(dvs,0)):

        sDs = sD(temperature, matrix, dvs[i])
    
        for p in range(0,np.size(matrix,0)):
            alpha_vs_p[p,0] = 1-np.exp((-sDs[p,0]*density*height)/(1+(sDs[p,0]*density*height)/(betas[p,0]*P_es[p,0]))**0.5)

        alpha_vs[i,0] = np.sum(alpha_vs_p)

    return alpha_vs

alpha_vs_co2 = alpha_vs(co2,dvs_co2,0.0004,11000)

plt.figure()
plt.plot(dvs_co2[:,0],alpha_vs_co2[:,0], label = 'spectral absorbance for CO2 (400ppm)')
plt.xlabel('wavenumber')
plt.ylabel('absorbance')
plt.legend(loc='upper right')
plt.show(block = False)

##def alpha_vs (temperature,matrix,dvs,pressure,partial_pressure,path_length):
##    alpha_vs = np.zeros((np.size(dvs,0),1))
##    global alpha_v
##    for i in range(0,np.size(dvs,0)):
##        alpha_vs[i] = alpha_v(temperature,matrix,dvs[i],pressure,partial_pressure,path_length)
##
##    return alpha_vs
